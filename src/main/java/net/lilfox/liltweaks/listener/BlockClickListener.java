...
public class BlockClickListener implements AttackBlockCallback {
    ...
    public ActionResult interact(PlayerEntity player, World world, Hand hand, BlockPos blockPos, Direction direction) {
        ...
        if ((block == Blocks.SMALL_AMETHYST_BUD || block == Blocks.MEDIUM_AMETHYST_BUD || block == Blocks.LARGE_AMETHYST_BUD) && Configs.saveKiddyAmethyst) {
            return ActionResult.SUCCESS;
        } else if (block == Blocks.BUDDING_AMETHYST && Configs.buddingAmethystBreakingRestriction) {
            return ActionResult.SUCCESS;
        } else if (Configs.permaHaste && blockState.calcBlockBreakingDelta(player, world, blockPos) >= 0.7F && blockState.calcBlockBreakingDelta(player, world, blockPos) < 1.0F) {
