...
public class NiceBalls {
    ...
    private Predicate<Entity> overrideTargetedEntityCheck(Predicate<Entity> predicate) {
        if (Configs.niceBalls.getBooleanValue() && this.client.player != null && !this.client.player.isSneaking() && (this.client.player.getStackInHand(Hand.MAIN_HAND).getItem() == Items.SNOWBALL || this.client.player.getStackInHand(Hand.MAIN_HAND).getItem() == Items.BOW)) {
